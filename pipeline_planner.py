...
/***************************************************************************
 ...
 Generated by Plugin Builder: http://g-sherman.github.io/Qgis-Plugin-Builder/
        ...
        begin                : 2023-12-05
        git sha              : $Format:%H$
        copyright            : (C) 2023 by Maria Marinez
        email                : maria.martinez@hotmail.co.uk
 ***************************************************************************/
...
class PipelinePlanner:
    ...
    def __init__(self, iface):
        ...
        self.dlg.tblImpacts.setColumnWidth(2,200)
    ...
    def evaluatePipeline(self, point, button):
        if button==Qt.LeftButton:
            ...
            for raptor in raptors:
                ...
                valDistance=pipeline.distance(raptor.geometry().centroid())#This will be the centre of the raptor buffer
                if raptor.geometry().intersects(pipeline):
                    row=self.dlg.tblImpacts.rowCount() # First we create a variable called row that hold the table current row count
                    self.dlg.tblImpacts.insertRow(row)# We want this to be the last row on the table
                    ...
                    self.dlg.tblImpacts.setItem(row, 3, QTableWidgetItem("{:4.5f}".format(valDistance)))#Distance is a floating pointnumber and we will format it as a stringso: 4 digits to the left of the decimal and 5 to the right
             #   QMessageBox.information(None,"Pipeline", pipeline.asWkt())

            for eagle in eagles:
                valConstraint="BAE Nest" # There is only one type so we just put a string
                valID=eagle.attribute("nest_id")
                valStatus=eagle.attribute("status")
                valDistance=pipeline.distance(eagle.geometry().centroid())#This will be the centre of the eagle buffer
            ...
            for buowl in buowls:
                valConstraint="BUOWL Habitat" # There is only one type so we just put a string
                valID=buowl.attribute("habitat_id")
                valStatus=buowl.attribute("recentstat")
                valDistance=pipeline.distance(buowl.geometry().buffer(-0.001,5))#This will be the centre of the eagle buffer
